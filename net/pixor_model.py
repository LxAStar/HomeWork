...
import net.se_module as se_module
from config.config_dict import get_default_config

Config = get_default_config()


class SplitOutputPixor(nn.Module):
    def __init__(self, Config):
        super(SplitOutputPixor, self).__init__()
        self.pixor = PIXOR(Config)
...
class Decoder(nn.Module):
    def __init__(self, Config, calculate_box=False):
        super(Decoder, self).__init__()
        self.geometry = [Config.geometry.width_min,
                         Config.geometry.width_max,
                         Config.geometry.length_min,
                         Config.geometry.length_max]
        self.grid_size = Config.geometry.grid_size

        self.target_mean    = Config.dataset.reg.mean#[0.008, 0.001, 0.202, 0.2, 0.43, 1.368]
        self.target_std_dev = Config.dataset.reg.std#[0.866, 0.5, 0.954, 0.668, 0.09, 0.111]
        self.calculate_box = calculate_box
    ...
    def forward(self, x):
        ...
        x = torch.arange(self.geometry[2], self.geometry[3], self.grid_size, dtype=torch.float32,
                         device=device)
        y = torch.arange(self.geometry[0], self.geometry[1], self.grid_size, dtype=torch.float32,
                         device=device)
        ...
        if self.calculate_box:
            ...
            decoded_reg = torch.cat([centre_x, centre_y, l, w, cos_t, sin_t], dim=1)
        return decoded_reg
