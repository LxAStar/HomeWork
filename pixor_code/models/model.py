...
class Decoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.labels_shape = config['labels_shape']
        ...
        self.target_std_dev = config['target_std_dev']
        self.y_min = config['geometry'][0]
        self.y_max = config['geometry'][1]
        self.x_min = config['geometry'][2]
        self.x_max = config['geometry'][3]
        self.h_min = config['geometry'][4]
        self.h_max = config['geometry'][5]
    ...
    def forward(self, x):
        ...
        x = torch.arange(self.labels_shape[1],
                         dtype=torch.float32,
                         device=device)
        y = torch.arange( - self.labels_shape[0] / 2,
                          self.labels_shape[0] / 2,
                          dtype=torch.float32,
                          device=device)
        x *= self.grid_size
        y *= self.grid_size
        ...
        decoded_reg = torch.cat([rear_left_x, rear_left_y,
                                 rear_right_x, rear_right_y,
                                 front_right_x, front_right_y,
                                 front_left_x, front_left_y], dim=1)
...
class PIXOR(nn.Module):
    ...
    def __init__(self, config, decode=False):
        ...
        self.backbone = BackBone(Bottleneck, [3, 6, 6, 3], config['use_bn'])
        self.header = Header(config['use_bn'])
