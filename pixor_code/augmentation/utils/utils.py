...
def crop_cloud(cloud, config, labels_Frange, remove_ground=True):
    """
        Args:
            cloud - point cloud
    ...
    out_points = cloud
    if remove_ground:
        # out_points, _ = remove_groundplane(cloud)
        pass

    out_points = out_points[out_points[:, 0] < config['geometry'][3]]
    out_points = out_points[out_points[:, 0] > config['geometry'][2]]
    out_points = out_points[out_points[:, 1] > config['geometry'][0]]
    out_points = out_points[out_points[:, 1] < config['geometry'][1]]

    if labels_range is not None:
        out_points = out_points[out_points[:, 0] < labels_range[3]]
        out_points = out_points[out_points[:, 0] > labels_range[2]]
        out_points = out_points[out_points[:, 1] > labels_range[0]]
        out_points = out_points[out_points[:, 1] < labels_range[1]]

    return out_points

#
# def remove_groundplane(points):
#     """
#         Args:
...
#     max_iter = 3
#     iter_counter = 0
#     horizontal_thresh = 0.9
#     temp_points = points[:, :3]
#     while not get_ground and iter_counter < max_iter:
#         cloud = pcl.PointCloud(temp_points)
...
#         try:
#             if abs(model[2]) > horizontal_thresh:  # check that we find ground (not some wall)
#                 get_ground = True
#                 avg_z = np.mean(temp_points[inliers][:, 2])
#             else:
...
#     # обрезает пол облака
#     if get_ground:
#         out_points = points[points[:, 2] > avg_z + 0.25]
#         return out_points, avg_z
#     else:
#         print('Warning: could\'nt find stable ground')
#         return points, None