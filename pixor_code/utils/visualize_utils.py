...
def plot_label_map(label_map):
    ...
    plt.figure()
    plt.imshow(np.rot90(np.squeeze(label_map, axis=0), 2))
    plt.show()


def build_box_plot(target_list, pred_list):
    plt.gcf()
    ...
    # rect_p = [patches.Rectangle(np.array(obj.bbox_center[:2]) - np.array(obj.bbox_size[:2]) / 2,
    #                             obj.bbox_size[0], obj.bbox_size[1],
    #                             linewidth=1, edgecolor='r', facecolor='none') for obj in pred_list]
    # rect_t = [patches.Rectangle(np.array(obj.bbox_center[:2]) - np.array(obj.bbox_size[:2]) / 2,
    #                             obj.bbox_size[0], obj.bbox_size[1],
    #                             linewidth=1, edgecolor='g', facecolor='none') for obj in target_list]

    rect_p = [patches.Rectangle(obj.bbox_center[:2],
                                obj.bbox_size[0], obj.bbox_size[1],
                                linewidth=1, edgecolor='r', facecolor='none') for obj in pred_list]
    rect_t = [patches.Rectangle(obj.bbox_center[:2],
                                obj.bbox_size[0], obj.bbox_size[1],
                                linewidth=1, edgecolor='g', facecolor='none') for obj in target_list]
    for rect in [*rect_p, *rect_t]:
        ax.add_patch(rect)
    plt.show()